...
RSpec.describe Kudzu::Crawler do
  ...
  let(:config_file_browser) { Rails.root.join('config/kudzu_browser.rb') }
  ...
  context "run with browser" do
    it "single thread" do
      crawler = Kudzu::Crawler.new(thread_num: 1, browser: true)
      crawler.run(seed_url) do
        on_success do |page, link|
          puts "on_success: #{page.status} #{page.url}"
        end
    ...
    # it 'multi thread' do
    #   crawler = Kudzu::Crawler.new(thread_num: 2, browser: true)
    #   crawler.run(seed_url)
    #   expect(crawler.repository.page.size > 0).to be_truthy
    # end

    # it 'with configs' do
    #   crawler = Kudzu::Crawler.new(config_file: config_file_browser)
    #   crawler.run(seed_url)
    #   expect(crawler.repository.page.size > 0).to be_truthy
    # end

    # it 'with uuid' do
    #   crawler = Kudzu::Crawler.new(uuid: SecureRandom.uuid, browser: true)
    #   crawler.run(seed_url)
    #   expect(crawler.repository.page.size > 0).to be_truthy
    # end
